...
class EntityAPIHandlerTest extends TestCase
{
    ...
    public function createProduct()
    {
        ...
        var_dump($record);
        // ZCRMModule::getInstance("Invoices");
    }
    ...
    public function downloadFile($moduleAPIName, $startTime, $endTime)
    {
        ...
        try {
            ...
            $endTime = microtime(true) * 1000;
            if ($responseIns->getHttpStatusCode() != APIConstants::RESPONSECODE_OK || count($responseIns->getData()) == 0) {
            ...
            Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMRecord('.self::$firstParnetModule.','.self::$firstParnetId.')', 'getAttachments()', 'Attachments fetched successfully', 'Attachments fetched='.count($responseIns->getData()), 'success', ($endTime - $startTime));
        } catch (ZCRMException $e) {
            $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
            Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'EntityAPIHandlerTest('.self::$firstParnetModule.')', 'downloadFile', $e->getMessage(), $e->getTraceAsString(), 'failure', ($endTime - $startTime));
    ...
    public function getNotes($moduleAPIName, $startTime, $endTime)
    {
        ...
        try {
            ...
            Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMRecord('.self::$firstParnetModule.','.self::$firstParnetId.')', 'getNotes()', 'Related Notes fetched Successfully', 'Notes Count='.count($responseIns->getData()), 'success', ($endTime - $startTime));
        } catch (ZCRMException $e) {
            $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;

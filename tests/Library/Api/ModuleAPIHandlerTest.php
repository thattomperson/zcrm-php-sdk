    ...
    public function testGetAllFields()
    {
        ...
        foreach (MetaDataAPIHandlerTest::$moduleList as $moduleApiName=>$moduleName) {
            ...
            try {
                ...
                if ($zcrmFields == null || count($zcrmFields) <= 0) {
                ...
                foreach ($zcrmFields as $zcrmField) {
                    ...
                    if (!is_bool($zcrmField->isCustomField()) || !is_bool($zcrmField->isVisible()) || !is_bool($zcrmField->isReadOnly()) || !is_bool($zcrmField->isBusinessCardSupported()) || !is_string($zcrmField->getFieldLabel()) || !is_int($zcrmField->getId()) || !is_string($zcrmField->getApiName()) || !is_int($zcrmField->getLength()) || !is_string($zcrmField->getCreatedSource()) || !is_string($zcrmField->getDataType()) || (!(TestUtil::isActivityModule($moduleName) || $moduleName == 'Activities') && $zcrmField->getJsonType() == null)) {
                ...
                Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getAllFields()', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
            }
        }
    }
        ...
        foreach (self::$moduleVsFieldIdMap as $moduleApiName=>$fieldList) {
            ...
            foreach ($fieldList as $field) {
                ...
                try {
                    ...
                    if (!is_bool($zcrmField->isCustomField()) || !is_bool($zcrmField->isVisible()) || !is_bool($zcrmField->isReadOnly()) || !is_bool($zcrmField->isBusinessCardSupported()) || !is_string($zcrmField->getFieldLabel()) || !is_int($zcrmField->getId()) || !is_string($zcrmField->getApiName()) || !is_int($zcrmField->getLength()) || !is_string($zcrmField->getCreatedSource()) || !is_string($zcrmField->getDataType()) || ($moduleName != 'Events' && $zcrmField->getJsonType() == null)) {
                    ...
                    $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
                    Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getFieldDetails('.$field.')', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
                }
            }
        }
    }
    ...
    public function testGetAllLayouts()
    {
        ...
        foreach (MetaDataAPIHandlerTest::$moduleList as $moduleApiName=>$moduleName) {
            ...
            try {
                ...
                $endTime = microtime(true) * 1000;
                if ($zcrmLayouts == null || count($zcrmLayouts) <= 0) {
                    throw new ZCRMException('Layout data is not Received');
                }
                $layoutIdList = [];
                foreach ($zcrmLayouts as $zcrmLayout) {
                    if ($zcrmLayout->getId() == null || $zcrmLayout->getName() == null || $zcrmLayout->getAccessibleProfiles() == null || $zcrmLayout->getSections() == null || !(count($zcrmLayout->getSections()) >= 1)) {
                        throw new ZCRMException('Invalid Layout Data Received');
                    }
                    if (!is_string($zcrmLayout->getName()) || !is_int($zcrmLayout->getId()) || !is_int($zcrmLayout->getStatus()) || !is_bool($zcrmLayout->isVisible())) {
                ...
                $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
                Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getAllLayouts()', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
            }
        }
    }
        ...
        foreach (self::$moduleVsLayoutIdMap as $moduleApiName=>$layoutList) {
            ...
            foreach ($layoutList as $layoutId) {
                ...
                try {
                    ...
                    $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
                    Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getLayoutDetails('.$layoutId.')', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
                }
            }
        }
    }
    ...
    public function testGetAllCustomViews()
    {
        ...
        foreach (MetaDataAPIHandlerTest::$moduleList as $moduleApiName=>$moduleName) {
            ...
            try {
                ...
                $endTime = microtime(true) * 1000;
                if ($zcrmCustomViews == null || count($zcrmCustomViews) <= 0) {
                ...
                foreach ($zcrmCustomViews as $zcrmCustomView) {
                    ...
                    } elseif (!TestUtil::isActivityModule($moduleName) && ($zcrmCustomView->getFields() == null || !(count($zcrmCustomView->getFields()) >= 1))) {
                ...
                $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
                Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getAllCustomViews', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
            }
        }
    }
        ...
        foreach (self::$moduleVsCustomViewIdMap as $moduleApiName=>$customViewIdList) {
            ...
            foreach ($customViewIdList as $customViewId) {
                ...
                try {
                    ...
                    if ($zcrmCustomView->getFields() != null && count($zcrmCustomView->getFields()) != 0) {
                    ...
                    $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
                    Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getCustomViewDetails('.$customViewId.')', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
                }
            }
        }
    }
    ...
    public function testGetAllRelatedLists()
    {
        ...
        foreach (MetaDataAPIHandlerTest::$moduleList as $moduleApiName=>$moduleName) {
            ...
            try {
                ...
                $endTime = microtime(true) * 1000;
                if ($zcrmModuleRelatedLists == null || count($zcrmModuleRelatedLists) <= 0) {
                ...
                $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
                Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getAllRelatedLists()', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
            }
        }
    }
        ...
        foreach (self::$moduleVsRelatedListIdMap as $moduleApiName=>$relatedListIds) {
            ...
            foreach ($relatedListIds as $relatedListId) {
                ...
                try {
                    ...
                    $endTime = $endTime == 0 ? microtime(true) * 1000 : $endTime;
                    Helper::writeToFile(self::$filePointer, Main::getCurrentCount(), 'ZCRMModule('.$moduleApiName.')', 'getRelatedListDetails('.$relatedListId.')', $e->getMessage(), count($e->getExceptionDetails()) > 0 ? $e->getExceptionDetails() : $moduleApiName, 'failure', ($endTime - $startTime));
                }
            }
        }
    }
